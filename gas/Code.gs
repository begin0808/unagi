...
function doPost(e) {
  try {
    ...
    try {
      ...
      sheet.appendRow([
        orderNo, now, name, phone ? "'" + phone : '', addressCell, email, lineId ? "'" + lineId : '',
    ...
    const payDeadline = payment !== 'cash'
...
function paymentLabel_(payment) {
  if (payment === 'cash') return '取貨時付現';
  if (payment === 'linepay') return 'LINE Pay';
  return '銀行轉帳';
}
...
function paymentInstructions_(o) {
  ...
  if (o.payment === 'linepay') {
    return [
      ...
      '確認收款後即安排出貨。',
      '',
      '※ 本店只會透過 LINE Pay 收款，不會要求您改匯到其他帳戶。如收到可疑訊息，請勿付款，並直接回覆本信確認。',
    ].join('\n');
  }
  const lines = ['【付款資訊】'];
  if (o.bank) {
  ...
  lines.push('');
  lines.push('※ 本店不會以電話或 LINE 通知更改匯款帳號。如收到類似通知，請勿匯款，並直接回覆本信確認。');
  return lines.join('\n');
}
  ...
  MailApp.sendEmail({
    ...
    body: [
      ...
      o.boxes > 0
        ...
        : '',
      '※ 鰻魚接近尾部稍有細刺，食用時請留意；給小朋友吃請由大人先分切。',
      '',
      '如有任何問題，我們會盡快與您聯繫。',
      '如訂單內容有誤，請直接回覆本信件告知。',
      '',
