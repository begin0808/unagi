...
import { 
  ...
  Utensils, ExternalLink, Anchor, ToggleLeft, ToggleRight 
} from 'lucide-react';
            ...
            <div className="bg-gradient-to-r from-red-900/60 to-stone-900 p-6 rounded-2xl border border-red-500/30 mb-10 relative overflow-hidden">
               ...
               <div className="flex flex-col md:flex-row items-center justify-center gap-2 md:gap-6">
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="text-4xl md:text-5xl font-bold text-amber-500">特價 $1200</span>
                    <span className="text-xl text-stone-300">/ 一公斤</span>
                  </div>
               </div>
            </div>
...
// --- 風格 2: 清新海洋風 (Light Theme) ---
const AppFresh = ({ scrollToSection, isMenuOpen, setIsMenuOpen }) => {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-700 font-sans selection:bg-sky-200 selection:text-sky-900 animate-fade-in">
      {/* Navigation */}
...
// -----------------------------------------------------------------------------
// Main App (風格切換器 Switcher)
// -----------------------------------------------------------------------------
const MainApp = () => {
  // 狀態管理：true 為職人黑金風，false 為清新海洋風
  const [isDarkTheme, setIsDarkTheme] = useState(true);
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // 當切換主題時，更新網頁標題
  useEffect(() => {
    document.title = isDarkTheme ? "興旺蒲燒鰻 - 職人黑金" : "興旺蒲燒鰻 - 清新海洋";
  }, [isDarkTheme]);
  ...
  return (
    <div className="relative">
      {/* 左下角浮動切換按鈕 */}
      <div className="fixed bottom-6 left-6 z-[9999] opacity-90 hover:opacity-100 transition-opacity">
        <button
          onClick={() => setIsDarkTheme(!isDarkTheme)}
          className={`
            flex items-center gap-3 px-6 py-4 rounded-full shadow-2xl font-bold transition-all duration-300 transform hover:scale-105 border-4
            ${isDarkTheme 
              ? 'bg-white text-stone-900 border-amber-600 hover:bg-stone-100' 
              : 'bg-stone-900 text-white border-sky-500 hover:bg-stone-800'}
          `}
        >
          {isDarkTheme ? <ToggleLeft size={28} className="text-amber-600" /> : <ToggleRight size={28} className="text-sky-400" />}
          <span className="text-lg hidden md:inline">
            {isDarkTheme ? "切換至：清新海洋風 🌊" : "切換至：職人黑金風 🔥"}
          </span>
          <span className="text-lg md:hidden">
            {isDarkTheme ? "切換風格 🌊" : "切換風格 🔥"}
          </span>
        </button>
      </div>
